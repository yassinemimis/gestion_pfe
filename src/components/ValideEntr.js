...
function ValideEntr() {
  ...
  useEffect(() => {
    axios
      .get("http://127.0.0.1:8000/api/enattente2")
      .then((response) => {
              ...
              <tr key={project.id_theme}>
                ...
                <td>
                  <button
                    ...
                    onClick={() => handleUpdate(project.id_theme, "accepted",3)}
                  >
                    acceptation
                  </button>
                  <button
                    className="btn btn-danger btn-sm mx-1"
                    onClick={() => handleUpdate(project.id_theme, "rejected",3)}
                  >
                    rejeter
                  </button>
                </td>
